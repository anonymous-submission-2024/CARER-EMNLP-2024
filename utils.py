...
class EHRDatasetNote:
    ...
    def _load(self, label):
        ...
        if label == 'm':
            ...
            y = np.load(os.path.join(self.path, 'hypertension_y.npz'))['hypertension_y']
...
class EHRDatasetNoteBert:
    def __init__(self, data_path, label='m', batch_size=32, shuffle=True, device=torch.device('cpu')):
        super().__init__()
        self.path = data_path
        self.code_x, self.visit_lens, self.y, self.divided, self.neighbors, self.note, self.attention_mask = self._load(label)
    ...
    def _load(self, label):
        ...
        divided = load_sparse(os.path.join(self.path, 'divided.npz'))
        neighbors = load_sparse(os.path.join(self.path, 'neighbors.npz'))
        note = np.load(os.path.join(self.path, 'note_x_bert.npz'), allow_pickle=True)['note_x']
        attention_mask = np.load(os.path.join(self.path, 'note_attention_mask.npz'), allow_pickle=True)['note_attention_mask']

        # print(type(note[0]))
        # print(type(note))
        return code_x, visit_lens, y, divided, neighbors, note, attention_mask
    ...
    def __getitem__(self, index):
        ...
        note = torch.from_numpy(self.note[slices]).to(device)
        note = torch.from_numpy(self.note[slices]).to(device)
        attn_mask = torch.from_numpy(self.attention_mask[slices]).to(device)
        return code_x, visit_lens, divided, y, neighbors, note, attn_mask
