...
from preprocess import save_sparse, save_data, save_data_notes, save_data_notes_bert
from preprocess.parse_csv_filter import Mimic3Parser, Mimic4Parser, EICUParser
from preprocess.encode import encode_code, encode_note_train, encode_note_test, encode_note_bert
from preprocess.build_dataset import split_patients, build_code_xy, build_heart_failure_y, build_note_x, build_note_x_bert
...
def read_option():
    ...
    parser.add_argument('--max_note_len', help='max length of clinical note', type=int, default=4096) #mimic3 or mimic4
...
if __name__ == '__main__':
    ...
    # dataset_path = os.path.join('data_alt', dataset)
    ...
    if not os.path.exists(raw_path):
        ...
        exit()
    parsed_path = os.path.join(dataset_path, 'parsed_alt')
    if from_saved:
    ...
    train_note_encoded = encode_note_bert(patient_note, train_pids, max_note_len=max_note_len)
    # # print(train_note_encoded.shape)
    valid_note_encoded = encode_note_bert(patient_note, valid_pids,  max_note_len=max_note_len)
    test_note_encoded = encode_note_bert(patient_note, test_pids,  max_note_len=max_note_len)
    ...
    train_note_x, train_note_attention_mask = build_note_x_bert(train_pids, train_note_encoded, max_word_num_in_a_note=4096)
    valid_note_x, valid_note_attention_mask = build_note_x_bert(valid_pids, valid_note_encoded, max_word_num_in_a_note=4096)
    test_note_x, test_note_attention_mask = build_note_x_bert(test_pids, test_note_encoded, max_word_num_in_a_note=4096)
    
    print('train_note_x',train_note_x.shape)
    print('train_note_attention_mask',train_note_x.shape)
    ...
    print('building train hypertension labels ...')
    train_hypertension_y = build_heart_failure_y('401', train_codes_y, code_map)
    print('building valid hypertension labels ...')
    valid_hypertension_y = build_heart_failure_y('401', valid_codes_y, code_map)
    print('building test hypertension labels ...')
    test_hypertension_y = build_heart_failure_y('401', test_codes_y, code_map)

    encoded_path = os.path.join(dataset_path, 'encoded_alt')
    ...
    standard_path = os.path.join(dataset_path, 'standard_alt')
    ...
    import numpy as np
    np.savez(os.path.join(train_path, 'hypertension_y'), hypertension_y=train_hypertension_y)
    np.savez(os.path.join(valid_path, 'hypertension_y'), hypertension_y=valid_hypertension_y)
    np.savez(os.path.join(test_path, 'hypertension_y'), hypertension_y=test_hypertension_y)

    print('\tsaving training data')
    save_data_notes_bert(train_path, train_code_x, train_visit_lens, train_codes_y, train_hf_y,train_diabetes_y, train_divided, train_neighbors,train_note_x, train_note_attention_mask)
    # print('\tsaving valid data')
    save_data_notes_bert(valid_path, valid_code_x, valid_visit_lens, valid_codes_y, valid_hf_y,valid_diabetes_y, valid_divided, valid_neighbors,valid_note_x, valid_note_attention_mask)
    print('\tsaving test data')
    save_data_notes_bert(test_path, test_code_x, test_visit_lens, test_codes_y, test_hf_y,test_diabetes_y, test_divided, test_neighbors, test_note_x, test_note_attention_mask)
