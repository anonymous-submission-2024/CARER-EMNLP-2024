...
def evaluate_codes_bert(model, dataset, loss_fn, output_size=1, historical=None):
    model.eval()
    total_loss = 0.0
    labels = dataset.label()
    outputs = []
    preds = []
    for step in range(len(dataset)):
        code_x, visit_lens, divided, y, neighbors, note,mask_tensor = dataset[step]
        # note = note[:,:1024]
        # mask_tensor = note != 0
        ...
        with torch.cuda.amp.autocast(enabled=True):
            output = model(code_x, divided, neighbors, visit_lens,note, mask_tensor).squeeze(1)
        loss = loss_fn(output, y)
        total_loss += loss.item() * output_size * len(code_x)
...
def evaluate_hf(model, dataset, loss_fn, output_size=1, historical=None, prior=None):
    model.eval()
    total_loss = 0.0
    labels = dataset.label()
    outputs = []
    ...
    print(report)
    return avg_loss, f1_score_, auc, report


def evaluate_hf_note(model, dataset, loss_fn, output_size=1, historical=None):
    model.eval()
    total_loss = 0.0
    labels = dataset.label()
    outputs = []
    preds = []
    for step in range(len(dataset)):
        code_x, visit_lens, divided, y, neighbors, note = dataset[step]
        note = note[:,:1024]
        mask_tensor = note != 0
        mask_tensor = mask_tensor.long()
        # print(mask_tensor, mask_tensor.shape)
...
def evaluate_hf_bert(model, dataset, loss_fn, output_size=1, historical=None):
    model.eval()
    total_loss = 0.0
    labels = dataset.label()
    outputs = []
...
def evaluate_hf_custom(model, dataset, loss_fn, output_size=1, historical=None):
    model.eval()
    total_loss = 0.0
    labels = dataset.label()
    outputs = []
    ...
    for step in range(len(dataset)):
        code_x, visit_lens, divided, y, neighbors = dataset[step]
        output = model(code_x,visit_lens).squeeze(1)
        loss = loss_fn(output, y)
        total_loss += loss.item() * output_size * len(code_x)
        output = output.detach().cpu().numpy()
        outputs.append(output)
